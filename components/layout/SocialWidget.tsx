...
export function SocialWidget() {
  ...
  return (
    <div className={`social-widget fixed right-5 bottom-5 z-40 ${open ? "is-open" : ""}`}>
      <div className="relative h-10 w-10">
        {items.map((item) => (
          ...
          >
            <a
              ...
              className={`social-widget-orb absolute top-1/2 left-1/2 flex h-11 w-11 items-center justify-center rounded-full text-white shadow-[0_8px_18px_rgba(23,32,43,0.22)] ${item.className}`}
              style={{
                ["--icon-rot" as string]: `-${item.angle}deg`,
                ["--icon-x" as string]: `-${RADIUS}px`,
              }}
            >
              <item.Icon />
            </a>
        ...
        <button
          ...
          className="relative z-10 flex h-10 w-10 items-center justify-center rounded-full bg-brand text-white shadow-[0_10px_24px_rgba(15,117,188,0.38)] transition-colors hover:bg-brand-dark"
        >
          <svg
            viewBox="0 0 24 24"
            className="social-widget-plus h-4 w-4"
            fill="none"
            aria-hidden="true"
          >
